    ...
    <div className="flex flex-col w-full">
      ...
      <section className="relative min-h-[85vh] flex items-center justify-center overflow-hidden bg-secondary">
        ...
        <div className="container relative z-10 mx-auto px-4 sm:px-6 lg:px-8 flex flex-col items-center text-center max-w-4xl pt-20">
          <span className="text-primary font-medium tracking-widest uppercase mb-4 animate-in fade-in slide-in-from-bottom-4 duration-700">
          ...
          <h1 className="text-5xl md:text-7xl lg:text-8xl font-serif font-bold text-foreground mb-6 leading-[1.1] tracking-tight animate-in fade-in slide-in-from-bottom-8 duration-700 delay-150 fill-mode-both">
            ...
            <span className="text-primary italic">{t("home.hero.titleHighlight")}</span>
          </h1>
          ...
          <div className="flex flex-col sm:flex-row gap-4 animate-in fade-in slide-in-from-bottom-8 duration-700 delay-500 fill-mode-both">
            ...
            <Link href="/catering" className="inline-flex items-center justify-center rounded-md text-sm font-medium transition-colors focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring border border-input bg-background shadow-sm hover:bg-accent hover:text-accent-foreground h-12 px-8 py-3 text-base">
              {t("home.hero.cateringEvents")}
            </Link>
          </div>
        </div>
      </section>
      ...
      <section className="py-12 border-y border-border/50 bg-card">
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8 text-center divide-x divide-border/50 rtl:divide-x-reverse">
            <div className="flex flex-col items-center justify-center">
              <span className="text-4xl font-serif font-bold text-primary mb-2">{stats?.totalProducts || "50"}+</span>
              <span className="text-sm text-muted-foreground uppercase tracking-wider">{t("home.stats.artisanGoods")}</span>
            </div>
            <div className="flex flex-col items-center justify-center">
              <span className="text-4xl font-serif font-bold text-primary mb-2">{stats?.totalCategories || "6"}</span>
              <span className="text-sm text-muted-foreground uppercase tracking-wider">{t("home.stats.categories")}</span>
            </div>
            <div className="flex flex-col items-center justify-center">
              <span className="text-4xl font-serif font-bold text-primary mb-2 flex items-center gap-1">
                {stats?.averageRating ? Number(stats.averageRating).toFixed(1) : "4.9"}
                <Star className="h-6 w-6 fill-primary" />
              </span>
              <span className="text-sm text-muted-foreground uppercase tracking-wider">{t("home.stats.averageRating")}</span>
            </div>
            <div className="flex flex-col items-center justify-center">
              <span className="text-4xl font-serif font-bold text-primary mb-2">{stats?.totalReviews || "500"}+</span>
              <span className="text-sm text-muted-foreground uppercase tracking-wider">{t("home.stats.happyCustomers")}</span>
            </div>
          </div>
        </div>
      </section>
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-end mb-12 border-b border-border pb-6">
            <div className="max-w-2xl">
              <h2 className="text-3xl md:text-5xl font-serif font-bold text-foreground mb-4">{t("home.featured.title")}</h2>
              <p className="text-muted-foreground text-lg">{t("home.featured.subtitle")}</p>
            </div>
            <Link href="/menu" className="hidden md:flex items-center gap-2 text-primary font-medium hover:underline hover:underline-offset-4 pb-2 shrink-0">
              {t("home.featured.viewMenu")} <ArrowRight className="w-4 h-4 rtl:rotate-180" />
            </Link>
          </div>
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col lg:flex-row gap-16 items-center">
            <div className="lg:w-1/2 relative">
              ...
              <div className="absolute -bottom-8 end-[-2rem] w-2/3 aspect-square rounded-lg overflow-hidden shadow-xl border-8 border-background z-20 hidden md:block">
            ...
            <div className="lg:w-1/2 space-y-8">
              <div>
                <span className="text-primary font-medium tracking-widest uppercase mb-2 block">{t("home.philosophy.label")}</span>
                <h2 className="text-4xl md:text-5xl font-serif font-bold text-foreground mb-6">{t("home.philosophy.title")}</h2>
              ...
              <Link href="/about" className="inline-flex items-center justify-center rounded-md text-sm font-medium transition-colors focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring border border-primary text-primary shadow-sm hover:bg-primary hover:text-primary-foreground h-11 px-8">
                {t("home.philosophy.readStory")}
              </Link>
            </div>
          </div>
        </div>
      ...
      <section className="py-24 bg-background">
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-16 max-w-2xl mx-auto">
            <span className="text-primary font-medium tracking-widest uppercase mb-2 block">{t("home.topRated.label")}</span>
            <h2 className="text-3xl md:text-5xl font-serif font-bold text-foreground mb-6">{t("home.topRated.title")}</h2>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
            {categories?.map(category => (
              <Link key={category.id} href={`/menu?category=${category.id}`} className="group flex flex-col items-center justify-center p-6 bg-background rounded-lg border border-border/50 hover:border-primary transition-colors hover:shadow-md">
                ...
                <span className="font-medium text-foreground group-hover:text-primary transition-colors text-center text-sm">{category.name}</span>
              </Link>
            ))}
          </div>
        </div>
      ...
      <section className="py-24 bg-primary text-primary-foreground relative overflow-hidden">
        ...
        <div className="container relative z-10 mx-auto px-4 sm:px-6 lg:px-8 text-center max-w-3xl">
          <h2 className="text-4xl md:text-5xl font-serif font-bold mb-6">{t("home.cateringCta.title")}</h2>
          <p className="text-lg md:text-xl text-primary-foreground/90 mb-10 font-light">{t("home.cateringCta.subtitle")}</p>
          <Link href="/catering" className="inline-flex items-center justify-center rounded-md text-sm font-medium transition-colors focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring bg-background text-primary shadow hover:bg-background/90 h-12 px-8 py-3 text-base">
            {t("home.cateringCta.cta")}
          </Link>
        </div>
      </section>
      ...
      <section className="py-24 bg-background">
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-12 text-center divide-y md:divide-y-0 md:divide-x divide-border rtl:md:divide-x-reverse">
            <div className="flex flex-col items-center p-6">
              <div className="w-12 h-12 rounded-full bg-primary/10 flex items-center justify-center mb-6">
                <MapPin className="w-6 h-6 text-primary" />
              </div>
              <h3 className="text-xl font-serif font-bold mb-2">{t("home.visit.visitUs")}</h3>
              <p className="text-muted-foreground mb-4">{t("home.visit.address")}</p>
              <Link href="/find-us" className="text-primary font-medium hover:underline">{t("home.visit.getDirections")}</Link>
            </div>
            ...
            <div className="flex flex-col items-center p-6 pt-12 md:pt-6">
              ...
              <h3 className="text-xl font-serif font-bold mb-2">{t("home.visit.hours")}</h3>
              <ul className="text-muted-foreground space-y-2">
                <li><strong className="text-foreground font-medium">{t("home.visit.satThu")}:</strong> {t("home.visit.satThuHours")}</li>
                <li><strong className="text-foreground font-medium">{t("home.visit.fri")}:</strong> {t("home.visit.friHours")}</li>
              </ul>
            </div>
            ...
            <div className="flex flex-col items-center p-6 pt-12 md:pt-6">
              ...
              <h3 className="text-xl font-serif font-bold mb-2">{t("home.visit.contact")}</h3>
              <p className="text-muted-foreground mb-4">{t("home.visit.contactSubtitle")}</p>
              <div className="space-y-2">
                <a href="tel:+966112345678" className="block text-primary font-medium hover:underline" dir="ltr">+966 11 234 5678</a>
